...
describe('CredentialsFlow', () => {
	...
	describe('#getToken', () => {
		...
		it('should request the token', async () => {
			...
			const nockScope = mockTokenCall({ requestedScope: 'notifications' });
			...
			const { headers, body } = await nockScope;
		...
		it('when scopes is an empty array, it should send empty scope string to an auth server', async () => {
			...
			const nockScope = mockTokenCall({ requestedScope: '' });
